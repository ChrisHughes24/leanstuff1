import data.complex.basic data.real.cau_seq tactic.norm_num data.nat.basic algebra.big_operators data.set.finite tactic.ring
...
lemma complex.exp_series_cau (x : ℂ) : is_cau_seq complex.abs (series (λ m, monoid.pow x m / fact m)) := begin
  cases exists_nat_gt (complex.abs x) with n hn,have n_pos : (0 : ℝ) < n := lt_of_le_of_lt (complex.abs_nonneg _) hn,
  refine series_ratio_test n (complex.abs x / n) _ _ _,exact div_nonneg_of_nonneg_of_pos (complex.abs_nonneg _) n_pos,rwa [div_lt_iff n_pos,one_mul],
  assume m mn,
  unfold monoid.pow fact,simp only [complex.abs_div,complex.abs_mul,div_eq_mul_inv,mul_inv',nat.cast_mul,complex.abs_inv],
  have : complex.abs x * complex.abs (monoid.pow x m) * ((complex.abs ↑(fact m))⁻¹ * (complex.abs ↑(succ m))⁻¹) = complex.abs x * complex.abs (monoid.pow x m) * (complex.abs ↑(fact m))⁻¹ * (complex.abs ↑(succ m))⁻¹,ring,rw this,
  have : complex.abs x * (↑n)⁻¹ * (complex.abs (monoid.pow x m) * (complex.abs ↑(fact m))⁻¹) = complex.abs x * complex.abs (monoid.pow x m) * (complex.abs ↑(fact m))⁻¹ * (↑n)⁻¹,ring,rw this,
