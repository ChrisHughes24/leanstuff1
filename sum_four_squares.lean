...
| 1 := λ hd, begin
  have h₁ := coprime.eq_one_of_dvd (coprime.pow n n hd) h,
...
begin
  ...
  { have : ∀ i j : ℤ, i * i - j * j = (i - j) * (i + j) := λ i j, by ring,
    have h₃ : p ∣ b.val - a.val ∨ p ∣ b.val + a.val := by
      rwa [nat.modeq.modeq_iff_dvd, int.coe_nat_mul, int.coe_nat_mul, this, ← int.coe_nat_sub h_1,
        ← int.coe_nat_add, ← int.coe_nat_mul, int.coe_nat_dvd, hp.dvd_mul] at h,
    by_contra hab,
    have hab' := fin.vne_of_ne hab,
    cases h₃ with h₃ h₃,
    { have : p ≤ b.1 - a.1 := le_of_dvd (nat.pos_of_ne_zero (λ h₂, hab' 
        (le_antisymm h_1 (nat.sub_eq_zero_iff_le.1 h₂)))) h₃, 
      have : p < p := 
        calc p ≤ b.1 - a.1 : this
    ...
    { have : 0 < b.1 + a.1,
      { cases a.1,
        ...
        exact succ_pos _ },
      have : p ≤ b.1 + a.1 := le_of_dvd this h₃,
      have : p < p :=
        calc p ≤ b.1 + a.1 : this
...
begin
  ...
  { rw [mod_sub_half, if_pos h, 
      abs_of_nonneg (int.mod_nonneg (n : ℤ) hm0)],
    exact h },
...
have h₁ : m = 1 := classical.by_contradiction $ λ h₁,
